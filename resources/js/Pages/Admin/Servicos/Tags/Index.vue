...
<template>
  <admin-layout>
    ...
    <template v-slot:header>
      <h5 class="font-weight-bolder text-white mt-4 mb-0">Tags</h5>
    </template>
                  ...
                  <tbody>
                    <tr v-for="(tag, index) in tags.data" :key="index">
                      ...
                      <td>
                        <div class="px-4 py-1">
                          <div class="justify-content-center">
                            <h6 class="mb-0 text-sm">{{ tag.descricao }}</h6>
                            <!--        <p class="text-xs text-secondary mb-0">
                              john@creative-tim.com
                            </p> -->
                          </div>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editMode === false ? "Adicionar Tag" : "Editar Tag";
    },
    buttonTxt() {
  ...
  methods: {
    ...
    create() {
      this.form.post(this.route("admin.tags.store"), {
        preserveScroll: true,
    ...
    update() {
      this.form.patch(
        this.route("admin.tags.update", this.form.id, this.form),
        {
          preserveScroll: true,
          onSuccess: () => {
            this.closeModal();
    ...
    deleteModal(tag) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.form.delete(this.route("admin.tags.destroy", tag), {
            preserveScroll: true,
            onSuccess: () => {
              Swal.fire("Deletado!", "Tag deletada", "success");
            },
          });
        }
      });
  ...
  mounted() {
    this.modal = new bootstrap.Modal(document.getElementById("modalTag"), {});
  },
};
</script>
